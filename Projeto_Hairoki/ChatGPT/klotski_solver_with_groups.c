...
static void parse_input(const char *filename, Board *init) {
    ...
    /* if next non-empty line is a number, parse group count */
    ln = next_trimmed_line(f);
    if (ln) {
        if (isdigit((unsigned char)ln[0])) {
            ...
            /* maybe a group line without count — harmless: try to parse as single group */
            parse_group_line(groups_count, ln);
            groups_count++;
            free(ln);
            /* read rest lines maybe (but we keep simple) */
        }
    }
    fclose(f);
}
