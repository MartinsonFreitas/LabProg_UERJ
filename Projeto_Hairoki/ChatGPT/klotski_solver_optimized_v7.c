...
/* Retorna a próxima linha 'trimmed' (sem espaços iniciais/finais), ou NULL no EOF.
   NÃO ignora linhas que comecem com '#': devolve-as para que o parser decida. */
...
/* Parser robusto que usa next_trimmed_line().
   - pula comentários antes do tabuleiro inicial;
   - lê tabuleiro inicial até encontrar "#solucao";
   - lê tabuleiro objetivo até encontrar "#grupos" ou EOF. */
static void parse_input(const char *filename, Board *init) {
    ...
    /* --- pular comentários iniciais e encontrar primeira linha do tabuleiro inicial --- */
    char *ln = NULL;
    while (1) {
        ln = next_trimmed_line(f);
        if (!ln) die("Arquivo terminou antes do tabuleiro inicial");
        if (ln[0] == '#') { free(ln); continue; } // comentário de cabeçalho -> pular
        // primeira linha válida do tabuleiro
        break;
    }
    // agora 'ln' contém a primeira linha do tabuleiro inicial
    if (r < MAX_ROWS) { lines[r++] = ln; int L = strlen(ln); if (L > maxc) maxc = L; } else { free(ln); die("Tabuleiro inicial grande demais"); }

    /* --- ler o restante do tabuleiro inicial até '#solucao' --- */
    while (1) {
        ln = next_trimmed_line(f);
        ...
        if (ln[0] == '#') {
            ...
            // comentário embutido -> ignorar e continuar
            free(ln);
            continue;
        }
