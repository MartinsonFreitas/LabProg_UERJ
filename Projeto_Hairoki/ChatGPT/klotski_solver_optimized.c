...
static void parse_input(const char *filename, Board *init) {
    ...
    // Ler linhas iniciais coletando matriz inicial até linha que inicia com #solução
    char *lines[MAX_ROWS]; int r = 0, maxc = 0;
    while (1) {
        char *ln = next_noncomment_line(f);
        if (!ln) break;
        if (strncmp(ln, "#solução", 8) == 0 || strncmp(ln, "#solucao", 8) == 0) { free(ln); break; }
        // se era comentário começando com # e NÃO é #solução, ignorar
        if (ln[0] == '#') { free(ln); continue; }
        // linha de tabuleiro
        lines[r] = ln; int L = strlen(ln); if (L > maxc) maxc = L; r++;
        if (r >= MAX_ROWS) break;
    }
    if (r == 0) die("arquivo sem tabuleiro inicial");
    init->rows = r; init->cols = maxc;
    // preenche board com '.' ou '0'? Enunciado usa '.' para vazio; manter '.'
    for (int i = 0; i < init->rows; ++i) for (int j = 0; j < init->cols; ++j) init->board[i][j] = '.';
    for (int i = 0; i < r; ++i) {
        for (int j = 0; j < (int)strlen(lines[i]); ++j) init->board[i][j] = lines[i][j];
        free(lines[i]);
    }

    // Ler matriz solução
    char *goal_lines[MAX_ROWS]; int gr = 0; int gmaxc = 0;
    while (1) {
        char *ln = next_noncomment_line(f);
        if (!ln) break;
        if (ln[0] == '#') { free(ln); continue; }
        goal_lines[gr] = ln; int L = strlen(ln); if (L > gmaxc) gmaxc = L; gr++;
        if (gr >= MAX_ROWS) break;
    }
    if (gr == 0) die("arquivo sem solucao");
    goal_rows = gr; goal_cols = gmaxc;
    for (int i = 0; i < goal_rows; ++i) for (int j = 0; j < goal_cols; ++j) goal_board[i][j] = '0';
    for (int i = 0; i < gr; ++i) {
        for (int j = 0; j < (int)strlen(goal_lines[i]); ++j) goal_board[i][j] = goal_lines[i][j];
        free(goal_lines[i]);
    }

    // Ler # grupos (podemos ignorar grupos para a versão básica)
    // simplesmente consumir resto do arquivo se houver
    fclose(f);
}
