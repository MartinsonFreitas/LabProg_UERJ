 ...
 * next_trimmed_line:
 * - lê linha
 * - remove \n/\r e espaços nas bordas
 * - retorna NULL no EOF
 * - NÃO filtra comentários (#...) — isso será feito no parse_input
 */
static char *next_trimmed_line(FILE *f)
{
    ...
    size_t s = 0;
    while (s < len && isspace((unsigned char)buf[s]))
        s++;
...
/*
 * PARSER UNIVERSAL DO HAIKORI
 * Suporta ambos formatos:
 *   A) objetivo seguido por "#grupos"
 *   B) objetivo seguido diretamente por número G
 *
 * Regras:
 *  - linhas vazias são ignoradas
 *  - linhas "#..." são ignoradas, EXCETO "#solucao" (ou "#solução")
 *  - objetivo termina quando encontramos:
 *       (1) "#grupos"   OU
 *       (2) linha contendo SOMENTE dígitos (ex: "4") → início dos grupos
 */
static void parse_input(const char *filename, Board *init)
{
    ...
    /* ============ LER MATRIZ INICIAL ATÉ "#solucao" ============ */
    while (1) {
        char *ln = next_trimmed_line(f);
        if (!ln) die("Faltou #solucao no arquivo");

        if (ln[0] == '\0') { free(ln); continue; }
        if (ln[0] == '#') {
            if (strncmp(ln, "#solucao", 8) == 0 || strncmp(ln, "#solução", 9) == 0) {
                free(ln);
                break; /* fim da matriz inicial */
            }
            free(ln);
            continue;
        }

        if (ri >= MAX_ROWS) die("Tabuleiro inicial grande demais");
    ...
    for (int i = 0; i < ri; i++) {
        ...
        for (int j = 0; j < maxci; j++)
            init->board[i][j] = (j < L ? lines_init[i][j] : '.');
        free(lines_init[i]);
    }
    ...
    while (1) {
        ...
        if (ln[0] == '#') {
            if (strncmp(ln, "#grupos", 7) == 0) {
                free(ln);
                break; /* fim da matriz objetivo */
            }
            free(ln);
            continue;
        }

        /* CHECK: linha somente com dígitos -> início dos grupos */
        bool digits_only = true;
        for (int k = 0; ln[k]; k++)
            if (!isdigit((unsigned char)ln[k])) { digits_only = false; break; }

        if (digits_only) {
            saved_group_line = ln;  /* linha do número G */
            break;
        }

        /* linha normal da matriz objetivo */
        if (ro >= MAX_ROWS) die("Tabuleiro objetivo grande demais");
    ...
    if (ro == 0) die("arquivo sem solucao");
    ...
    if (saved_group_line) {
        /* caso B: linha NUMÉRICA já lida */
        int G = atoi(saved_group_line);
        free(saved_group_line);
        ...
        /* tentar ler "#grupos" ou número */
        char *ln = next_trimmed_line(f);
        if (!ln) return;

        if (ln[0] == '#') {
            /* formato A: "#grupos" seguido por G */
            free(ln);
            ...
            for (int g = 0; g < G; g++) {
                char *l3 = next_trimmed_line(f);
                if (!l3) break;
                parse_group_line(groups_count, l3);
                free(l3);
                groups_count++;
            }
        }
        else {
            /* formato B mas chegou por aqui */
            bool digits_only = true;
            for (int k = 0; ln[k]; k++)
                if (!isdigit((unsigned char)ln[k])) { digits_only = false; break; }
