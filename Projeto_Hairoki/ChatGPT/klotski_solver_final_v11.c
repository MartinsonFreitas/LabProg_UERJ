...
/* ----------------- Replacement: robust parse_input -----------------
   Detecta "#solucao" mesmo com acentos/variações, e aceita formato
   com ou sem "#grupos" (linha numérica G).
*/
static void parse_input(const char *filename, Board *init) {
    ...
    /* --- Ler tabuleiro inicial até encontrar linha que comece com "#sol" --- */
    while (1) {
        char *ln = next_trimmed_line(f);
        if (!ln) die("arquivo sem #solucao");
        if (ln[0] == '#') {
            /* use prefix "#sol" para pegar "#solucao" ou "#solução" etc */
            if (strncmp(ln, "#sol", 4) == 0) { free(ln); break; }
            free(ln);
            continue;
        }
    ...
    /* --- Ler objetivo: até "#grup" ou linha só com dígitos (G) --- */
    ...
    while (1) {
        ...
        if (!ln) break;
        if (ln[0] == '#') {
            if (strncmp(ln, "#grup", 5) == 0) { free(ln); break; } /* aceita "#grupos" */
            free(ln); continue;
        }
        /* se linha contém só dígitos -> é a contagem de grupos (arquivo sem "#grupos") */
        bool all_digits = true;
        for (size_t p = 0; ln[p]; ++p) if (!isdigit((unsigned char)ln[p])) { all_digits = false; break; }
        if (all_digits) { saved_group_line = ln; break; }
