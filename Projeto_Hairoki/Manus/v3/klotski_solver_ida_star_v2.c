...
// Armazena apenas o estado para verificar ciclos no caminho atual
...
void add_visited(const State *state, HashTable hash_table);
void remove_visited(const State *state, HashTable hash_table);
...
// --- Funções de Hashing e Visitação (Adaptadas para Backtracking Removal) ---
...
// Retorna true se o estado for encontrado no caminho atual
bool is_visited(const State *state, HashTable hash_table) {
...
void add_visited(const State *state, HashTable hash_table) {
    ...
    // Não verifica se já existe, pois o is_visited é chamado antes.
    // Se o estado já existe, é um ciclo e a busca é podada.
...
// Remove o estado do Hash Set (Backtracking Removal)
void remove_visited(const State *state, HashTable hash_table) {
    unsigned long index = hash_state_optimized(state);
    HashNode *current = hash_table[index];
    HashNode *prev = NULL;
...
// --- Algoritmo IDA* (Iterative Deepening A*) com Backtracking Removal ---
        ...
        for (int d = 0; d < 4; d++) {
            ...
            if (move_piece(current_state, piece_id, directions[d], &next_state)) {
                ...
                if (solution_found) {
                    ...
                    remove_visited(current_state, visited);
                    return result;
                }
...
void solve_klotski_ida_star(const char *filename) {
    ...
    printf("Iniciando busca IDA* com Backtracking Removal...\n");

    while (limit <= max_limit && !solution_found) {
        printf("Iteração com limite F = %d\n", limit);
        
        // O Hash Set é limpo a cada iteração, mas o Backtracking Removal
        // garante que ele nunca cresça além da profundidade de busca.
        // A limpeza aqui é apenas para garantir que a iteração comece limpa.
        ...
        add_visited(&initial_state, visited);
        
        int next_limit = search_ida_star(&initial_state, 0, limit, visited);
        
        // Remove o estado inicial do Hash Set após a iteração
        remove_visited(&initial_state, visited);
        
        if (solution_found) {
            printf("Solução encontrada em %d movimentos (IDA* com Backtracking Removal):\n", limit);
    ...
    // A memória já foi liberada pelo Backtracking Removal e pela limpeza no loop.
    // free_hash_table(visited); // Comentado, pois já foi liberado.
}
