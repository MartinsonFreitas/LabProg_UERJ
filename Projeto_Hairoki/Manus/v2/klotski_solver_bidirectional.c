...
void create_goal_state(const State *initial_state, State *goal_state) {
    ...
    *goal_state = *initial_state;
    ...
    if (piece_X_goal) {
        piece_X_goal->row = target_row;
        piece_X_goal->col = target_col;
    }

    // 6. Desenha a peça 'X' na nova posição no tabuleiro do estado final
    for (int i = target_row; i < target_row + piece_X_initial->height; i++) {
        for (int j = target_col; j < target_col + piece_X_initial->width; j++) {
...
void solve_klotski(const char *filename) {
    ...
    memset(visited_fwd, 0, sizeof(visited_fwd));
    ...
    memset(visited_bwd, 0, sizeof(visited_bwd));
    Node *goal_node_start = create_node(&goal_state, NULL, '\0', '\0');
    enqueue(q_bwd, goal_node_start);
    mark_visited_optimized(&goal_state, goal_node_start, visited_bwd);
    ...
    while (!is_empty(q_fwd) && !is_empty(q_bwd)) {
        ...
        Queue *current_q;
        HashTable *p_current_visited, *p_other_visited;
        bool is_fwd_turn;

        if (q_fwd->front->state.move_count <= q_bwd->front->state.move_count) {
            current_q = q_fwd;
            p_current_visited = &visited_fwd;
            p_other_visited = &visited_bwd;
            is_fwd_turn = true;
        } else {
            current_q = q_bwd;
            p_current_visited = &visited_bwd;
            p_other_visited = &visited_fwd;
            is_fwd_turn = false;
        }
        ...
        for (int i = 0; i < current_state->num_pieces; i++) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if (move_piece(current_state, piece_id, directions[d], &next_state)) {
                    ...
                    Node *collision_node = get_visited_node(&next_state, *p_other_visited);
                    if (collision_node != NULL) {
                        Node *fwd_result, *bwd_result;
                        if (is_fwd_turn) {
                            fwd_result = current_node;
                            bwd_result = collision_node;
                        } else {
                            fwd_result = collision_node;
                            bwd_result = current_node;
                        }
                        // Reconstruir o caminho
                        reconstruct_path(fwd_result, bwd_result);
                        goto end_search;
                    }

                    // 2. Se não colidiu, continua a busca
                    if (!is_visited_optimized(&next_state, *p_current_visited)) {
                        Node *next_node = create_node(&next_state, current_node, piece_id, directions[d]);
                        mark_visited_optimized(&next_state, next_node, *p_current_visited);
