...
static int goal_matches(const Board *b, const Goal *g) {
    ...
    for (int r = 0; r < g->rows; ++r) {
        for (int c = 0; c < g->cols; ++c) {
            char goal = g->cells[r][c];
            char actual = b->cells[r][c];

            if (goal == '0') continue; // curinga
            if (goal == 'X' && actual != 'X') return 0; // peça alvo
            if (goal != 'X' && actual != goal) return 0; // outras peças
        }
    }
    return 1;
}
...
static Node *solve(const Board *start, const Goal *goal, const Groups *gr, NodePool *pool) {
    ...
    int stepsExplorados = 0; // contador de estados

    while (!q_empty(&q)) {
        Node *cur = q_pop(&q);
        stepsExplorados++;
        if (stepsExplorados % 1000 == 0) {
            printf("Explorados: %d estados...\n", stepsExplorados);
        }
