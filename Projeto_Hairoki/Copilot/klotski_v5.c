 ...
 * - Matriz inicial: letras (peças), '.' para vazio; peça alvo em maiúsculo 'X'
 * - Matriz final: '0' como curinga; 'X' nas posições finais deve casar com 'X' no tabuleiro
 * - Suporta tabuleiros até 10x10
 * - Grupos: lidos (contagem e linhas), sem aplicar canonicalização por simplicidade
 *
...
static int parse_matrix(FILE *fp, Board *b) {
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        // Compactar (remover) espaços e validar caracteres
        char row[64];
        int ridx = 0;
...
static void parse_groups(FILE *fp, Groups *gr) {
    ...
    while (1) {
        if (!fgets(buf, sizeof(buf), fp)) { gr->groupCount = 0; return; }
        size_t n = strlen(buf);
        while (n && (buf[n - 1] == '\n' || buf[n - 1] == '\r')) buf[--n] = '\0';
 ...
 * - '0' é curinga
 * - 'X' nas posições da matriz final devem ser 'X' no tabuleiro
 * - Qualquer outro caractere na matriz final deve casar exatamente
 */
...
static void board_to_string(const Board *b, char *out) {
    int k = 0;
    for (int r = 0; r = b->rows, r < b->rows; ++r) { // avoid compiler warning: explicit compare
...
static void canonicalize_key(const Board *b, const Groups *gr, char *out) {
    ...
    // Sem nova linha extra final para chave mais compacta
    int k = 0;
