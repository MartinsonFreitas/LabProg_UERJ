 ...
 * Características: A*, Canonicalização de Grupos, Visualização ANSI.
...
typedef struct {
    char grid[MAX_ROWS][MAX_COLS]; // Usado para verificação rápida de colisão
...
typedef struct Node {
    ...
    struct Node *parent;
    int g; // Custo real
    int h; // Heurística (CORRIGIDO: O erro estava na falta deste campo)
...
// Canonicalização: Ordena peças por grupo -> linha -> coluna
...
int calculate_heuristic(const State *s) {
    ...
    // CORRIGIDO: const Piece* para evitar warning
    ...
    // Penalidade se houver peças bloqueando o caminho para baixo
