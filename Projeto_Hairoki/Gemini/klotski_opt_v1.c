/* * klotski_opt.c - Solucionador Otimizado para Klotski
 * Implementa A* com Canonicalização de Grupos e Rastreamento Incremental de Estado.
 * * Compilar: gcc -O3 -o klotski_opt klotski_opt.c
 * Uso: ./klotski_opt engarrafamento.txt
 */
...
char* generate_canonical_key(const State *s) {
    // 1. Copia as peças para um array temporário
    Piece temp[MAX_PIECES];
    memcpy(temp, s->pieces, sizeof(Piece) * s->num_pieces);
    
    // 2. Ordena as peças baseada no grupo e posição
    // Isso garante que se duas peças do mesmo grupo trocarem de lugar,
    // a chave gerada será a mesma.
    qsort(temp, s->num_pieces, sizeof(Piece), compare_pieces);
    
    // 3. Serializa para string (formato: "id:r,c|id:r,c|...")
    // Como os IDs podem variar, usamos apenas a posição ordenada por grupo
    char *key = malloc(s->num_pieces * 10 + 10);
    char *ptr = key;
    
    for (int i = 0; i < s->num_pieces; i++) {
        // Codificamos: GroupID + Row + Col (o ID original 'a','b' é descartado na chave para garantir simetria)
...
int calculate_heuristic(const State *s) {
    ...
    // 2. Penalidade por peças bloqueando o caminho (Assumindo que o alvo quer descer)
    // Isso ajuda muito no Engarrafamento
    int penalty = 0;
        ...
        for (int r = target->r + target->h; r < s->rows; r++) {
            for (int c = target->c; c < target->c + target->w; c++) {
                if (s->grid[r][c] != '.' && s->grid[r][c] != TARGET_ID) {
                    penalty += 2; // Penaliza peças abaixo
                }
            }
        }
    }
...
// Retorna true se encontrou um estado com g menor ou igual (não processar)
// Retorna false se é novo ou melhor (processar)
bool hash_visited(HashEntry **table, const char *key, int g) {
    ...
    while (e) {
        if (strcmp(e->key, key) == 0) {
            if (e->g <= g) return true; // Já visitamos com custo melhor
            e->g = g; // Atualiza custo
            return false;
        }
        e = e->next;
    }
...
bool is_valid_move(const State *s, int p_idx, int dr, int dc) {
    ...
    // Verifica colisão apenas nas células novas
    // Truque: Verificar todas as células ocupadas pela peça na nova posição.
    // Se a célula não for vazia E não for ocupada pela própria peça, é colisão.
...
State apply_move(const State *current, int p_idx, int dr, int dc) {
    State next = *current; // Copia struct inteira (rápido para structs pequenas)
    Piece *p = &next.pieces[p_idx];
    
    // 1. Limpa posição antiga na grid
    for (int r = 0; r < p->h; r++) {
        for (int c = 0; c < p->w; c++) {
    ...
    // 3. Preenche nova posição na grid
    for (int r = 0; r < p->h; r++) {
        for (int c = 0; c < p->w; c++) {
...
void parse_input(const char *filename, State *initial) {
    ...
    char line[256];
    int row = 0;
    int mode = 0; // 0=Board, 1=Solution, 2=Groups
    
    // Init globals
    for(int i=0; i<256; i++) PIECE_GROUPS[i] = i; // Default: cada peça é seu próprio grupo
    initial->num_pieces = 0;
    initial->rows = 0;

    // Matriz temporária para detecção de peças
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (mode == 0) { // Lendo Tabuleiro
            ...
            row++;
            initial->rows = row;
        } else if (mode == 1) { // Lendo Solução (Detectar Alvo)
            ...
            row++;
        } else if (mode == 2) { // Lendo Grupos
             // Se a linha for só um número, ignora. Se tiver letras, processa.
             if (isdigit(line[0])) continue;
             
             // Assumimos que cada linha nesta seção define um grupo.
             // Ex: "a b c d" -> todos grupo X
             // Usaremos o ID do primeiro char como GroupID
             int group_id = -1;
             for (int i=0; line[i]; i++) {
                 if (isalpha(line[i])) {
                     if (group_id == -1) group_id = line[i]; // Use ASCII as ID
    ...
    // Processar peças do tabuleiro inicial
    bool visited[MAX_ROWS][MAX_COLS] = {0};
    
    // Copia para o grid do estado
    ...
    for(int r=0; r<initial->rows; r++) {
        for(int c=0; c<initial->cols; c++) {
            ...
            if (id != '.' && id != '0' && !visited[r][c]) {
                ...
                // Flood fill simples para achar largura/altura (assumindo retângulos)
                int w = 0, h = 0;
...
void solve(const char *filename) {
    ...
    int dr[] = {-1, 1, 0, 0};
    int dc[] = {0, 0, -1, 1};
    char dname[] = {'U', 'D', 'L', 'R'}; // Cima, Baixo, Esq, Dir
