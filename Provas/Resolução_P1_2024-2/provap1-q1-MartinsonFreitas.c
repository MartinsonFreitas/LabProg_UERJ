 ...
 * Implementação baseada no padrão do repositório (ImprimeMaiorLinha.c).
 */
int lelinha(char s[], int lim) {
    int c, i;
...
/*
 * contem_numero: verifica se a string contém algum dígito.
...
/*
 * processa_linha: extrai palavras, inverte se necessário e imprime
 * com formatação de 5 palavras por linha.
 */
void processa_linha(char linha[]) {
    char *token;
