...
int eh_primo(int n)
{
    // 0 e 1 não são primos. Números negativos também não.
    if (n <= 1) {
        return 0;
    }
    ...
    // Otimização: verificar divisores a partir de 5, de 6 em 6 (5, 7, 11, 13, ...)
    // pois todos os primos maiores que 3 são da forma 6k ± 1.
    int i;
    int limite = (int)sqrt(n);

    for (i = 5; i <= limite; i = i + 6) {
        if (n % i == 0 || n % (i + 2) == 0) {
            return 0;
        }
    }
