...
void encrypt_phrase(const char* phrase, const char* key, char* encrypted_phrase) {
    ...
    for (int i = 0; phrase[i] != '\0'; i++) {
        int value = get_char_value(phrase[i]);
        if (value > 0) {
...
void decrypt_phrase(const char* encrypted_phrase, const char* key, char* decrypted_phrase) {
    ...
    for (int i = 0; encrypted_phrase[i] != '\0'; i++) {
        ...
        if (current_key_index != -1) {
            ...
            if (original_value >= 1 && original_value <= 26) original_char = 'A' + original_value - 1;
            else if (original_value == 27) original_char = ',';
            else if (original_value == 28) original_char = '.';
            else if (original_value == 29) original_char = ' ';
            if (original_char != '\0') {
                decrypted_phrase[output_index++] = original_char;
            }
            prev_key_index = current_key_index;
        } else {
            decrypted_phrase[output_index++] = encrypted_phrase[i];
        }
    }
...
int ler_chave_de_arquivo(const char* filename, char* key) {
    ...
    if (fgets(key, KEY_SIZE + 2, fp) == NULL) { // Lê um pouco mais para checar o tamanho
        fprintf(stderr, "Erro ao ler a chave do arquivo ou arquivo vazio.\n");
        fclose(fp);
        return 0;
    }
    fclose(fp);
    key[strcspn(key, "\n")] = 0; // Remove quebra de linha se houver
    if (strlen(key) != KEY_SIZE) {
        fprintf(stderr, "Aviso: A chave no arquivo '%s' nao tem o tamanho esperado de %d caracteres.\n", filename, KEY_SIZE);
        return 0; // Considera um erro se o tamanho não for exato
    }
    return 1;
}
